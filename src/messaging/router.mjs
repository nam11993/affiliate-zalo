...
export class MessageRouter {
  ...
  async route(input, { mockAffiliate = null, previewCommands = false, skipDedupe = false } = {}) {
    ...
    if (message.isOwnMessage) return { action: 'NO_REPLY', reason: 'OWN_MESSAGE', message, parsed: null };
    if (this.allowedGroupIds.size && !this.allowedGroupIds.has(message.groupId)) return { action: 'NO_REPLY', reason: 'GROUP_NOT_ALLOWED', message, parsed: null };
    if (!skipDedupe && this.deduper.hasOrAdd(message)) return { action: 'NO_REPLY', reason: 'DUPLICATE_MESSAGE', message, parsed: null };
    ...
    if (parsed.type === 'SHOPEE_URL') {
      const result = await handleAffiliateLink({ message, parsed, mockAffiliate, processLink: this.processLink, classifyLink: this.classifyLink });
      return { ...result, message, parsed };
    }
    return {
      action: 'NO_REPLY',
