...
export function parseMessage(text) {
  ...
  if (commandMatch) {
    ...
    if (EXACT_COMMANDS.has(command)) {
      return args ? { type: 'UNKNOWN_COMMAND', command, args, reason: 'COMMAND_DOES_NOT_ACCEPT_ARGS' } : { type: 'COMMAND', command, args: '' };
    }
  ...
  if (shopeeUrls.length) return { type: 'SHOPEE_URL', urls: shopeeUrls, count: shopeeUrls.length };
