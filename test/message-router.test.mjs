...
test('router builds quote + mention + fixed affiliate template from mock data', async () => {
  ...
  const result = await router.route({
    senderId: 'user-1',
    senderName: 'Thanh Thanh',
    groupId: 'group-1',
    messageId: 'msg-1',
    text: 'https://shopee.vn/product/731999859/41261407193'
  }, {
    mockAffiliate: {
      affiliateUrl: 'https://s.shopee.vn/TEST123',
...
test('router ignores own message to prevent reply loops', async () => {
  const router = new MessageRouter();
  const result = await router.route({
    senderId: 'bot', groupId: 'group-1', messageId: 'msg-own', text: '#vitien', isOwnMessage: true
  });
...
test('router recognizes commands but does not activate phase-2 handler by default', async () => {
  const router = new MessageRouter();
  const result = await router.route({
    senderId: 'user-1', senderName: 'Trang', groupId: 'group-1', messageId: 'msg-cmd', text: '#vitien'
  });
...
test('router ignores normal chat', async () => {
  const router = new MessageRouter();
  const result = await router.route({
    senderId: 'user-1', groupId: 'group-1', messageId: 'msg-normal', text: 'xin chào mọi người'
  });
  assert.equal(result.action, 'NO_REPLY');
...
test('router rejects Shopee Video after a short link is resolved', async () => {
  const router = new MessageRouter();
  const result = await router.route({
    senderId: 'user-1', senderName: 'Nguyễn Hải Nam', groupId: 'group-1', messageId: 'msg-video',
    text: 'https://vn.shp.ee/58nvmrdm?smtt=0.0.9'
  }, {
    mockAffiliate: {
      resolvedUrl: 'https://sv.shopee.vn/share-video/demo?smtt=0.0.9',
      affiliateUrl: 'https://s.shopee.vn/SHOULD_NOT_USE', productName: 'x', commission: 1
    }
  });
  assert.equal(result.action, 'REPLY');
...
test('router rejects direct Shopee Live URLs', async () => {
  const router = new MessageRouter();
  const result = await router.route({
    senderId: 'user-1', senderName: 'Nam', groupId: 'group-1', messageId: 'msg-live',
    text: 'https://live.shopee.vn/share?from=live&viewer=1'
  }, { mockAffiliate: { affiliateUrl: 'https://s.shopee.vn/NO', productName: 'x', commission: 1 } });
  assert.equal(result.handler, 'unsupported-media-link');
...
test('router does not treat unresolved short link as a product in mock mode', async () => {
  const router = new MessageRouter();
  const result = await router.route({
    senderId: 'user-1', senderName: 'Nam', groupId: 'group-1', messageId: 'msg-short',
    text: 'https://vn.shp.ee/58nvmrdm?smtt=0.0.9'
  }, {
    mockAffiliate: {
      affiliateUrl: 'https://s.shopee.vn/SHOULD_NOT_USE', productName: 'x', commission: 1
...
test('router rejects mock resolved URL when it is still a short link', async () => {
  const router = new MessageRouter();
  const result = await router.route({
    senderId: 'user-1', senderName: 'Nam', groupId: 'group-1', messageId: 'msg-short2',
    text: 'https://vn.shp.ee/58nvmrdm?smtt=0.0.9'
  }, {
    mockAffiliate: {
      resolvedUrl: 'https://vn.shp.ee/58nvmrdm?smtt=0.0.9',
      affiliateUrl: 'https://s.shopee.vn/SHOULD_NOT_USE', productName: 'x', commission: 1
    }
  });
  assert.equal(result.action, 'NO_REPLY');
